...
public class Repository {
	...
	private HashTable<String, List<Article>> byAuthor;
	private HashTable<String, List<Article>> byKeyword;
	private HashTable<Integer, Article> byArticleId;
	...
	/**
	 * Adds a new article and propagates all the changes on the other data
	 * structures
	 * 
	 * @param a
	...
	public boolean insertArticle(Article a) {
		...
		try {
			...
			while (authors.hasNext()) {
				String name = authors.next(); // get its name
	...
	/**
	 * Checks if the article exists, if it does, finds its keywords and loops
	 * through the keywords articles. Once it finds the one to remove, does so
	 * and verifies that the keywords article list isn't empty. If it is,
	 * removes itself from the map. Repeats the process for authors
	 * 
	 * @param id
	...
	public void removeArticle(int id) {
		...
		// Lock the bucket where article with id is
	...
	public boolean validate() {
		...
		HashSet<String> authorIds = new HashSet<String>();
		...
		while (aIt.hasNext()) {
			...
			while (authIt.hasNext()) {
				...
				// if the author is different, add it to the set
				if (!authorIds.contains(name)) {
					authorIds.add(name);
					authorCount++;
				}
			}
	...
	/**
	 * Returns true if the author wrote the article and false otherwise
	 * 
	 * @param a
	 * @param author
	 * @return
